...
/**
 * MICROPROTOTYPE A+B "ASSEMBLY + SIGNAL".
 *
 ...
 * telemetria, grid/glow/vinheta — é IDÊNTICO ao original e à Direção B. A
 * única diferença é a cena 3D importada: `digital-core-scene-ab` estende a
 * montagem em um arco Fragmentação→Aproximação→Encaixe mais longo e legível
 * antes de entrar na coreografia de energia da Direção B (Ativação→Conexão→
 * Sistema vivo). Ver esse arquivo para o detalhe da coreografia.
 *
 * NÃO promovido a produção. Rota de comparação em `/lab/hero-refinement/ab`.
 */

const DigitalCoreScene = dynamic(
  () => import("./digital-core-scene-ab").then((mod) => mod.DigitalCoreSceneAB),
  { ssr: false },
);
