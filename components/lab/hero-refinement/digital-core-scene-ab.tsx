...
/**
 * HERO REFINEMENT — MICROPROTOTYPE A+B "ASSEMBLY + SIGNAL".
 *
 * Fork de `./digital-core-scene-b` (Direção B, preservada intacta em
 * `/lab/hero-refinement/b`). Geometria, materiais, câmera, iluminação e
 * scroll-exit continuam IDÊNTICOS ao original de produção — nada disso foi
 * tocado. Duas camadas foram reescritas em cima da B:
 *
 * 1) MONTAGEM (Fases 1–3: Fragmentação → Aproximação → Encaixe) — módulos
 *    partem de mais longe, com mais atraso relativo entre si e um easing
 *    mais "pesado" (easeOutQuart) do que o assembly original, para que a
 *    leitura seja "peças convergindo com pesos diferentes", não "tudo
 *    encaixa de uma vez". Ao final, um beat estrutural único e discreto
 ...
 *    tráfego de dados.
 * 2) ATIVAÇÃO + CONEXÃO (Fases 4–6) — depois de uma pausa curta e legível
 *    (T_ACTIVATE = T_COMPLETE_BEAT + 0.45s), o núcleo liga e a peça entra
 *    exatamente na coreografia da Direção B (heartbeat periódico + relays
 *    module→module), que se repete indefinidamente como estado de "sistema
 *    vivo". Essa camada não foi alterada, só reancorada a T_ACTIVATE em vez
 *    do T_ENERGY fixo da B.
 *
 * Ver relatório da Gate "A+B Microprototype 001" para o racional completo.
 */
...
/**
 * FASE 1 (Fragmentação) — distâncias de partida maiores que a B: os módulos
 * nascem visivelmente fora do enquadramento do núcleo, não a meio caminho.
 */
...
/** FASE 5 (Conexão) — mesmas relays module→module da Direção B, inalteradas. */
 ...
 * módulo quase 2x mais espaçados que a Direção B (que herdava o timing
 * curto do original de produção): dá tempo de ler "peças ainda separadas,
 * convergindo com pesos e ritmos diferentes" antes do encaixe.
 */
const T_PRIMARY = 0.46;
const T_SECONDARY = 0.85;
const T_STRUT = 1.23;
const T_PANEL = 1.27;
 ...
 * beat estrutural "sistema completo" (pulso central + rim response +
 * intensificação breve das conexões, ainda sem tráfego).
 */
...
/**
 * FASE 4 (Ativação) — pausa curta e legível antes do núcleo ligar. Substitui
 * o T_ENERGY fixo da Direção B por um marco relativo ao fim real da
 * montagem: a partir daqui a peça entra na coreografia da B inalterada
 * (heartbeat periódico + relays), repetida como "sistema vivo" (Fase 6).
 */
...
function ModularEngine({
  ...
  useFrame((state) => {
    ...
    if (coreGroupRef.current) {
      ...
      // "Pulso central" — o cluster inteiro do núcleo respira uma vez no beat de encaixe.
    ...
    modules.forEach((m, i) => {
      ...
      if (ref) {
        ...
        ref.visible = p > 0.002;
        // FASE 1 — leve aumento sobre a B na diferença de rotação durante a
        // aproximação (peças ainda "desalinhadas" enquanto convergem).
        const dock = (1 - p) * 0.68;
        ref.rotation.set(
          m.rotation[0] +
    ...
    // --- ENERGIA (Fases 4–6, herdadas da Direção B): batimento periódico
    // ancorado em T_ACTIVATE, não mais em T_ENERGY fixo. ---
    ...
    routes.forEach((r, i) => {
      ...
      // de encaixe ainda dá um brilho breve e residual às conexões (Fase 3:
      // "intensificação das conexões"), sem chegar a ser tráfego real.
