...
/**
 * STATE MACHINE — X03 INTERVAL, Phase B.
 *
 * Um único reducer tipado, sem booleans soltos. Cada fase (`Phase`) sabe
 * exatamente que frame mostrar (ver `frameForState` em use-x03-experience)
 * e se aceita a ação PRIMARY imediatamente ou só depois de um hold mínimo.
 *
 * Tabela de transições relevantes (STATE → EVENT → NEXT · SIDE EFFECT):
 *
 *   arrival        → PRIMARY              → calibrationA   · cutTo(A0)
 *   calibrationA   → PRIMARY (≥hold)      → calibrationB   · cutTo(B)
 *   calibrationB   → PRIMARY (≥hold)      → interTrial(0)  · cutTo(neutral)
 ...
 *   trialB(i)      → PRIMARY (≥hold)      → interTrial(i+1) se i+1<N, senão recall · session: advancedAt
...
export function autoDelayFor(state: ExperienceState): number | null {
  switch (state.phase) {
    case "interTrial": {
      ...
      return Math.max(160, TIMING.interTrial + drift);
    }
    case "recall":
...
function advance(state: ExperienceState, at: number): ExperienceState {
  switch (state.phase) {
    ...
    case "calibrationB":
      return { phase: "interTrial", nextIndex: 0, enteredAt: at };
    case "interTrial":
      return {
        phase: "trialA",
    ...
    case "trialB": {
      ...
      if (next < TRIAL_COUNT) return { phase: "interTrial", nextIndex: next, enteredAt: at };
      return { phase: "recall", enteredAt: at };
    }
    case "recall":
