/**
 * Gate 04B — Machine Perception Integration. Three REAL candidate paths
 * through the FIELD's obstacle field (field-environment.ts) — a "prefer
 * left", a "prefer right" and a "prefer center" A* run, each guaranteed by
 * construction to clear obstacles. Not fake offset lines: each candidate is
 * a genuinely viable route, so CONSIDER's "multiple futures existed" is true
 * of the geometry, not simulated. Ported from the Gate 04A discovery's
 * possibility-routes.ts (components/showcase/x03-lab/perception/), which
 * validated this exact mechanism.
 */
import { ALL_PIECES, CORRIDOR_X_HALF_WIDTH, CORRIDOR_Z_END, CORRIDOR_Z_START, ROUTE_GOAL, ROUTE_START } from "./field-environment";
 ...
 * one side of the corridor whenever it has a choice, while never crossing a
 * blocked cell — a genuinely different viable route, not a cosmetic offset
 * of the same one.
 */
...
/** The chosen route — center-biased shortest path, also the path Active
 *  Perception's attention travels during SEE/REMEMBER (see
 *  machine-perception-scene.tsx) — "the machine looks where it intends to
 *  go", perception and plan sharing one line. */
